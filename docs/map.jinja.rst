...
We will see a quick setup to configure the ``registry`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``registry`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``registry`` formulas under ``/srv/salt/registry/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/registry/parameters/dns:domain/
    ln -s dns:domain /srv/salt/registry/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/registry/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/registry/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``registry`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``registry`` formulas under ``/srv/salt/registry/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/registry/parameters/roles
...
We create a configuration for the role ``registry/server`` in ``/srv/salt/registry/parameters/roles/registry/server.yaml``:
...
We create another configuration for the role ``registry/client`` in ``/srv/salt/registry/parameters/roles/registry/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles registry/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles registry/client
...
.. code-block:: console
    ...
    mv /srv/salt/registry/parameters/dns:domain/  '/srv/salt/registry/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-registry-as-json:
      test.nop:
        - name: {{ registry | json }}
...
Here is an example based on `template-formula/registry/config/file.sls`_:
...
This ``sls`` file expose a ``registry`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ registry['winner'] }}
...
.. _template-formula/registry/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/registry/config/file.sls
